#include <stdint.h>
#include <stdlib.h>
#include <stdbool.h>
#include "ads8681.h"
#include "SPI.h"
#include "PINs.h"

uint32_t ADS8681_transfer_frame(uint32_t tx_frame)
{
	uint16_t rx1;
	uint16_t rx2;

	ADC_SPI_NCS = 0;

	rx1 = spi1_transfer16(tx_frame >> 16);
	rx1 = spi1_transfer16(tx_frame >> 16);
	
	ADC_SPI_NCS = 1;
	ADC_SPI_NCS = 0;
	rx2 = spi1_transfer16(tx_frame);

	ADC_SPI_NCS = 1;

	return ((uint32_t)rx1 << 16) | rx2;
}

uint32_t ADS8681_send_Command_blocking(
	enum ads868x_spi_command command,
	enum ads868x_register_address reg,
	uint16_t data)
{
	uint32_t frame;

	frame  = ((uint32_t)((command << 1) | ((reg >> 8) & 1))) << 24;
	frame |= ((uint32_t)(reg & 0xFF)) << 16;
	frame |= data;

	frame = ADS8681_transfer_frame(frame);
	return frame;
}

void ADS8681_init()
{
    ADC_NRESET = 0;

    volatile int i;
    for(i=0;i<10000;i++);

    ADC_NRESET = 1;

    uint16_t range_sel_reg;

    range_sel_reg = ADS8681_RANGE_SEL_UP_1_25_VREF;

    ADS8681_send_Command_blocking(
        ADS868X_SPI_COMMAND_WRITE_FULL,
        ADS868X_REGISTER_ADDRESS_RANGE_SEL,
        range_sel_reg
    );
}

double ADS8681_get_VOLT(uint16_t raw)
{
    return (((double)raw - 32768.0) / 32768.0) * 12.288;
}