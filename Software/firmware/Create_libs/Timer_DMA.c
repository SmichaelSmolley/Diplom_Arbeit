...
void DMA_tx_setup_SPI1_to_Mem(uint16_t* buffer, uint32_t numm_of_words)
{
	...
	DMA1_Channel3->CPAR = (uint32_t)&SPI1->DR; //Peripheral address DR-Register SPI1
	
	DMA1_Channel3->CMAR = (uint32_t)buffer; // Memory address
	
	DMA1_Channel3->CNDTR = numm_of_words;
		
	DMA1_Channel3->CCR =
      DMA_CCR1_DIR // richtung setzten mem to per
    | DMA_CCR1_MSIZE_0// Memory 16 bit
    | DMA_CCR1_PSIZE_0// Peripheral 16 bit
    | DMA_CCR1_PL_1// Priority high
		| DMA_CCR1_TCIE;// interrupt after end
...
void DMA_tx_disable()
{
	DMA1_Channel3->CCR &= ~DMA_CCR1_EN; // DMA enable
}
